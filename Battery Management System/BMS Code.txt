...
bool prevEmergencyButtonState = HIGH;     // Previous state of the emergency button
bool prevOutputButtonState = HIGH;        // Previous state of the output relay button
bool prevTemperatureButtonState = HIGH;   // Previous state of the temperature button
bool prevVoltageButtonState = HIGH;       // Previous state of the voltage button
...
void loop() {
  float temperature = getTemperature();   // Read the temperature
  ...
  bool currentEmergencyButtonState = digitalRead(emergencyButtonPin);
  if (currentEmergencyButtonState != prevEmergencyButtonState && currentEmergencyButtonState == LOW) {
  ...
  prevEmergencyButtonState = currentEmergencyButtonState;

  // Check output relay button
  bool currentOutputButtonState = digitalRead(outputButtonPin);
  if (currentOutputButtonState != prevOutputButtonState && currentOutputButtonState == LOW) {
  ...
  prevOutputButtonState = currentOutputButtonState;

  // Check temperature button
  bool currentTemperatureButtonState = digitalRead(temperatureButtonPin);
  if (currentTemperatureButtonState != prevTemperatureButtonState && currentTemperatureButtonState == LOW) {
  ...
  prevTemperatureButtonState = currentTemperatureButtonState;

  // Check voltage button
  bool currentVoltageButtonState = digitalRead(voltageButtonPin);
  if (currentVoltageButtonState != prevVoltageButtonState && currentVoltageButtonState == LOW) {
  ...
  prevVoltageButtonState = currentVoltageButtonState;
...
void displayTemperature(float temperatureC) {
  ...
  lcd.print(temperatureC,1);  // Display the temperature
...
void turnOffSystem() {
  ...
  digitalWrite(chargingRelayPin, HIGH);  // Turn off the charging relay
  digitalWrite(outputRelayPin, HIGH);    // Turn off the output relay
...
void toggleOutputRelay() {
  ...
  isOutputRelayManual = !isOutputRelayManual;  // Toggle the output relay manual flag
}
